...
export default function PublicProposal() {
  ...
  const accent = p.accent || '134,59,255';
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '3rem 1.25rem 6rem' }}>
        ...
        {/* Portada */}
        {p.coverImage && (
          <img src={p.coverImage} alt="" style={{ width: '100%', maxHeight: '260px', objectFit: 'cover', borderRadius: '1.1rem', marginBottom: '1.8rem', border: `1px solid rgba(${accent},0.2)` }} />
        )}
        ...
        {(p.ownerPhoto || p.ownerName) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1.8rem' }}>
            {p.ownerPhoto
              ...
              : <div style={{ width: '54px', height: '54px', borderRadius: '50%', background: `rgba(${accent},0.2)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', fontWeight: 800, color: `rgb(${accent})` }}>{(p.ownerName || '?')[0].toUpperCase()}</div>}
            <div>
              <div style={{ fontWeight: 800, fontSize: '1.05rem' }}>{p.ownerName}</div>
        ...
        {/* Hero */}
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 3rem)', fontWeight: 900, margin: '0.5rem 0 0', lineHeight: 1.1, letterSpacing: '-0.03em' }}>
          {p.title}
        </h1>
        {p.prospectName && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.7rem' }}>
              {p.gallery.map((img, i) => (
                <figure key={i} style={{ margin: 0 }}>
                  <img src={img.url} alt={img.caption || ''} style={{ width: '100%', borderRadius: '0.7rem', border: `1px solid rgba(${accent},0.2)`, display: 'block' }} />
            ...
            <div style={{ display: 'grid', gap: '0.6rem' }}>
              {p.callUrls.filter((c) => safeHttpUrl(c.url)).map((c, i) => (
                <a key={i} href={safeHttpUrl(c.url)} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '0.9rem 1rem', borderRadius: '0.7rem', background: 'rgba(255,255,255,0.04)', border: `1px solid rgba(${accent},0.2)`, color: 'white', textDecoration: 'none' }}>
                  <span style={{ width: '34px', height: '34px', borderRadius: '50%', background: `rgba(${accent},0.2)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: `rgb(${accent})`, fontWeight: 800 }}>▶</span>
          ...
          <Block accent={accent} label="Lo que detecté">
            <div style={{ display: 'grid', gap: '0.7rem' }}>
              {p.auditPoints.filter((a) => a.title).map((a, i) => (
                <div key={i} style={{ padding: '1rem 1.1rem', background: `rgba(${accent},0.07)`, borderLeft: `3px solid rgb(${accent})`, borderRadius: '0.5rem' }}>
        ...
        {(p.offer?.summary || p.offer?.price) && (
          <div style={{ marginTop: '2.5rem', padding: '1.8rem', borderRadius: '1.2rem', background: `linear-gradient(135deg, rgba(${accent},0.16), rgba(${accent},0.04))`, border: `1px solid rgba(${accent},0.35)` }}>
            ...
            {p.offer.price && (
              <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'baseline', gap: '0.6rem', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '1.8rem', fontWeight: 900, color: `rgb(${accent})` }}>{p.offer.price}</span>
          ...
          <Block accent={accent} label="Preguntas frecuentes">
            <div style={{ display: 'grid', gap: '0.7rem' }}>
              {p.objections.filter((o) => o.q).map((o, i) => (
                <div key={i} style={{ padding: '1rem 1.1rem', background: 'rgba(255,255,255,0.03)', borderRadius: '0.7rem', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <Block accent={accent} label="Lo que dicen">
            <div style={{ display: 'grid', gap: '0.7rem' }}>
              {p.testimonials.filter((t) => t.text).map((t, i) => (
                <blockquote key={i} style={{ margin: 0, padding: '1rem 1.2rem', borderRadius: '0.7rem', background: 'rgba(255,255,255,0.03)', borderLeft: `3px solid rgba(${accent},0.5)` }}>
...
function ctaBtn(accent, primary) {
  return {
    flex: '1 1 220px', padding: '1.1rem', borderRadius: '0.9rem', cursor: 'pointer',
    fontSize: '1rem', fontWeight: 800, border: primary ? 'none' : `1px solid rgba(${accent},0.4)`,
    background: primary ? `linear-gradient(135deg, rgb(${accent}), rgba(${accent},0.7))` : 'rgba(255,255,255,0.05)',
    color: 'white', boxShadow: primary ? `0 8px 28px rgba(${accent},0.4)` : 'none',
    fontFamily: 'inherit'
  };
}
...
function Centered({ children }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#09090b', color: '#94a3b8', fontFamily: "'Outfit', system-ui, sans-serif", padding: '2rem', textAlign: 'center' }}>
      <p>{children}</p>
    </div>
  );
}
